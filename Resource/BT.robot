...
Wait And Click
    ...
    [Arguments]   ${Locator}   ${Timeout}=10    # ${Timeout}=${Wait_Time}
...
Terminate Browsers
    [Documentation]   This keyword will kill the browser and terminate the browser process.
    [Timeout]   5 seconds
    Close All Browsers
...
Banner Count
    ...
    #${count}    Should Contain X Times      ${Banner_Class}     3
    should Be True  ${count} >= 3
    [Return]  ${count}

Validate Deals
    [Documentation]  This keyword will validate the sim only deals
    [Arguments]   ${Locator}   ${Timeout}=20    # ${Timeout}=${Wait_Time}    ${deals}
    run keyword and return status       Wait Until Element Contains       ${Deals}     was